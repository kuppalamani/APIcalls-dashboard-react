...
const cards = [
  ...
  { key: 'dailyAvg',        label: 'Daily Average',         icon: '⟡', color: '#a78bfa', sub: 'Calls / day' },
  { key: 'activeTenants',   label: 'Active Tenants',        icon: '⬡', color: '#34d399', sub: 'With traffic' },
  { key: 'totalConnectors', label: 'Connectors',            icon: '⊕', color: '#fbbf24', sub: 'Unique' },
  { key: 'thisMonth',       label: 'This Month',            icon: '◉', color: '#f87171', sub: 'Current period' },
];
      ...
      {cards.map(({ key, label, icon, color, sub }, i) => (
        ...
        >
          ...
          <div style={{
            ...
            background: color, borderRadius: '50%', opacity: 0.04, filter: 'blur(20px)',
          }} />
          <div style={{ fontSize: 22, marginBottom: 10, opacity: 0.6, color }}>{icon}</div>
          <div style={{
            fontFamily: "'JetBrains Mono', monospace",
            fontSize: 26, fontWeight: 600,
            color: '#f1f5f9', letterSpacing: '-0.02em', lineHeight: 1,
            marginBottom: 6,
          }}>
            {fmt(metrics[key] || 0)}
          </div>
