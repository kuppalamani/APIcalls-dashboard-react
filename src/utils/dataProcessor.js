...
export function getDateRange(records){
  ...
  if(!records || !records.length) return {minDate:null,maxDate:null};
...
export function getConnectorByTenant(records){
  ...
  records.forEach(r=>{
    ...
    map[tenant][connector]=(map[tenant][connector]||0)+(r.calls||0);
...
export function getHeatmapData(records){
  ...
  records.forEach(r=>{
    ...
    map[tenant][r.date]=(map[tenant][r.date]||0)+(r.calls||0);
...
export function getConnectorTrend(records){
  ...
  records.forEach(r=>{
    ...
    if(!map[connector]) map[connector]={};

    map[connector][r.date]=(map[connector][r.date]||0)+(r.calls||0);
